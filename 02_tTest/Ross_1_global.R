...
# ANALYSIS INFO ----
study.description      <- 'False Consensus 1 (Ross et al., 1977)'
analysis.unique.id     <- 43
analysis.name          <- 'Ross.1'
analysis.type          <- 1
...
ML2.df <- ML2.df %>%
  dplyr::select(2, 7, 24, 30, 521, 522, 523, 524, 525, 526, 527, 528, 529, 530, 531, 532, 535, 536, 537) %>%
...
ML2.var[[g]] <- varfun.Ross.1(ML2.sr[[g]])

stat.params <<- ML2.in$stat.params

t.test(variable1 ~ variable2, data = ML2.var[[g]]$cleanDataFilter, var.equal = FALSE)

#write.csv(ML2.df, file = file.path(project.root,"02_tTest","ross1Data"), row.names = FALSE)
...
# THIS IS STUDY-SPECIFFIC
original_study_estimated_effect_size <- 0.99
...
#plot(metaEType2Realizations[[99]], type = 'l')
#lines(metaFType2Realizations[[99]], col="red")
...
# THIS IS STUDY-SPECIFFIC
#saveRDS(all_results, file = file.path("~", "projects", "manyLabsE","02_tTest","Ross1Data.rds"))
