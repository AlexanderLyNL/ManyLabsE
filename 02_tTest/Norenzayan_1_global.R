...
# ANALYSIS INFO ----
study.description      <- 'Intuitive Reasoning (Norenzayan et al., 2002)'
analysis.unique.id     <- 59
analysis.name          <- 'Norenzayan.1'
analysis.type          <- 1
...
ML2.df <- ML2.df %>%
  dplyr::select(2, 7, 436, 441, 446, 451, 456, 461, 466, 471, 476, 481, 486, 491, 496, 501, 506, 511, 516, 521, 526, 531, 536, 541, 546, 551, 556, 561, 566, 571, 576, 581, 586, 591, 596, 601, 606, 611, 616, 621, 626, 631, 805, 904, 905, 906, 907, 908, 909, 910, 911, 912, 913, 914, 915, 938, 939, 940) %>%
...
ML2.var[[g]] <- varfun.Norenzayan.1(ML2.sr[[g]])
...
# THIS IS STUDY-SPECIFFIC
original_study_estimated_effect_size <- 0.69 # not sure about this
...
#plot(log(metaEType1), type = 'l')
#lines(log(metaFType1), col="red")

#plot(log(metaEType2), type = 'l')
#lines(log(metaFType2), col="red")

#plot(log(metaEType3), type = 'l')
#lines(log(metaFType3), col="red")
...
# THIS IS STUDY-SPECIFFIC
#saveRDS(all_results, file = file.path("~", "projects", "manyLabsE","02_tTest","NorenzayanData.rds"))
