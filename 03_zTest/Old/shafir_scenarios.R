...
mean(stops[is.finite(stops)])# 1.297
var(stops[is.finite(stops)])# 0.4152062
...
eValueMat <- eValueFutMat <- matrix(NA, n_sources, n_max)
...
compute_evalue_mats <- function(dat, allSources, designObj){
  ...
  for (s in seq_along(allSources)) {
    ...
    for (i in seq_len(n_obs)) {
      tempDat <- someDat[i, ]
      if (tempDat$variable2=="Award") {
        nAward <- nAward+1
        xAward <- tempDat$count+xAward
      } else if (tempDat$variable2=="Deny") {
        nDeny <- nDeny+1
        xDeny <- tempDat$count+xDeny
      }
...
mean(stops[is.finite(stops)])# 6.23
var(stops[is.finite(stops)])# 0.3405051


mean(type == "futility")# 1 times we stopped for futility
mean(type == "reject")# 0
...
mean(stops[is.finite(stops)]) # 201.88
var(stops[is.finite(stops)]) # 2026.814


mean(type == "futility")# 1
