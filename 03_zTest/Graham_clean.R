...
abline(v=mean(dat$variable1), col="red")
# hist(sd1Vec)
bep <- density(mean2Vec)
lines(bep$x, bep$y, col="blue")
abline(v=mean(dat$variable2), col="blue")

plot(hist(rVec))
abline(v=cor(dat$variable1, dat$variable2))
hist(sd2Vec)
...
for (g in unique(grahamData$cleanDataFilter$source)){
  z <- atanh(r1) / sqrt(1 / (n1 - 3))
  p <- 2 * (1 - pnorm(abs(z)))
