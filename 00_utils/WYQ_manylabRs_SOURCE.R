...
clean.ML2fieldsNA <- function(source.raw, pattern = "(test|-99)", S1 = TRUE, ps = "# ") {
  ...
  source.raw$Finished[which(c(
    "R_blS3XHmD4p14kf3",
    "R_8Cjo1Lguez90bqt",
    "R_bC85NEijsCjOqep"
  ) %in% source.raw$ResponseID)] <- 0

  textcolumns <- c("age", "sex", "hometown", "education", "comments", "raceother", "cogref.1", "cogref.2", "cogref.3")

  if (!S1) {
    textcolumns <- c(
      textcolumns, "ross.s2.1_1_TEXT", "ross.s2.1_2_TEXT", "sava1.2", "sava1.3", "sava1.7", "sava1.8",
  ...
  if (S1) {
    textcolumns <- c(
      textcolumns, "crit1.1_1_TEXT", "crit1.1_2_TEXT", "crit1.1_3_TEXT",
  ...
  disp(message = paste(
    ...
    collapse = ""
  ), header = "Clean ML2 Test Data - Step 2", footer = F)
  ...
  # Now look for '-99'
  disp(
    message = 'Checking all columns except "LocationLongitude" for pattern: "-99"',
...
get.cases <- function(rule, study.vars, study.vars.labels, stat.params) {
  ...
  isna <- ifelse(stat.params$censorNA,
    {
      " & !is.na(X)"
    },
    {
...
get.sourceData <- function(ML2.id, ML2.df, ML2.in) {
  ...
  for (i in seq_along(ML2.in$study.vars)) {
    ...
    suppressWarnings(if (
      any(eval(parse(text = paste0(
        "apply(",
        names(ML2.in$study.vars)[i], ",2,is.numeric)==FALSE"
      ))))) {
      eval(parse(text = paste0(
        names(ML2.in$study.vars)[i], " <- data.frame(sapply(which(apply(",
  ...
  for (i in seq_along(dfname)) {
    ...
    eval(parse(text = paste0(
      "attr(", names(ML2.in$study.vars)[[i]], ",'uID')",
...
get.oriESCI <- function(CL = .95) {
  ...
  for (s in ID.ori) {
    ...
    if (esType == "Z.f") {
      stat.test <- cor_test_fisherZ(
        r1 = ML2.ori$orig.stat.estimate1[[s]],
...
get.analyses <- function(studies = NA,
                         ...
                         indir = list(
                           RAW_DATA = ifelse(staticData, file.path(rootdir, "extdata", "RAW_DATA"), ""),
                         ...
                         outdir = list(
                           R_OBJECTS = ifelse(saveRdata, file.path(rootdir, "extdata", "R_OBJECTS"), ""),
                           RDS_RESULTS = ifelse(saveRdata, file.path(rootdir, "extdata", "RESULTS_RDS"), "")
                         ),
                         data.names = list(
                           Slate1 = ifelse(staticData, file.path(rootdir, "extdata", "RAW_DATA", "ML2_Rawdata_S1.RData"), "ML2_S1"),
  ...
  for (s in studs) {
    ...
    if (NROW(ML2.df) > 0) {
      ...
      if (Stud %in% "Rottenstreich") {
        Stud <- "Rottenstrich"
      }
      if (Stud %in% "Ross" & (ML2.key[s, "study.slate"] == 1)) {
        Stud <- "Ross.Slate1"
      }
      if (Stud %in% "Ross" & (ML2.key[s, "study.slate"] == 2)) {
        Stud <- "Ross.Slate2"
      }
      if (Stud %in% "vanLange") {
        Stud <- "VanLange"
      ...
      for (g in seq_along(runGroups)) {
        ...
        if (all(nMin1, nMin2)) {
          ...
          if (all(
            is.null(stat.test$warning), grepl("simpleWarning", stat.test$warning),
            ...
            if (tp < 4) {
              if (runGroups[g] != "all") {
                ...
                if (sum(sID) == 1) {
                  ...
                  SourceInfo2 <- raw.df[[g]] %>%
                    dplyr::filter(case.include) %>%
                    group_by(source) %>%
                    dplyr::summarise(
  ...
  if (outdir$RDS_RESULTS != "") {
    fname <- c(
      file.path(normalizePath(outdir$RDS_RESULTS), paste0("ML2_results_global_", subset, ".rds")),
      ...
      file.path(normalizePath(outdir$RDS_RESULTS), paste0("Data_Figure_StudyOrder_", subset, ".rds"))
    )[tp]

    saveRDS(list(
      raw.case = ML2.rawdata,
...
df.Clean <- function(df, Sep = ".") {
  ...
  ifelse(length(unique(nmsPP)) == length(nmsPP),
    {
      nms <- nmsPP
    },
    {
...
get.CSVdata <- function(path = NA, fID, finishedOnly = TRUE) {
  ...
  if (grepl(".xlsx", files)) {
    ...
    ifelse(is.data.frame(df$value),
      {
        df <- df$value
      },
      {
        df <- dplyr::tbl_df(read.csv(files, skip = 2, header = F, stringsAsFactors = F, col.names = t1, fileEncoding = "latin1"))
      }
    )
  }
...
get.zavCode <- function(df = NULL, lookup = NULL) {
  ...
  if (all(df[varsCOLD] == "")) {
    df$zav.condition <- "Hot"
  } else {
    df$zav.condition <- "Cold"
  }
  return(df)
}
...
get.descriptives <- function(stat.test, vars, keytable) {
  ...
  if (any(names(vars) == "df")) {
    vars$df <- NULL
  }
  if (any(names(vars) == "cleanDataFilter")) {
    vars$cleanDataFilter <- NULL
  }
  ...
  if (any(plyr::laply(vars, is.factor))) {
    ...
    descr.raw <- plyr::ldply(vars, function(ignore) {
      ...
      if (!is.data.frame(ignore)) {
        ignore <- data.frame(as.numeric(ignore))
      }
  ...
  if (esType == "f" | grepl("lm", keytable$stat.type)) {
    test <- cbind.data.frame(
      statistic = (eval(parse(text = keytable$stat.ncp))),
...
decide.analysis <- function(ML2.key, studies = NA, tp = NA, doAll = FALSE) {
  ...
  if (is.na(tp[1])) {
    disp(paste0("Analyzing global, primary and secondary studies marked '1' in the corrsponding columns of the masteRkey spreadsheet"), header = FALSE, footer = TRUE)
    studiess <- c(
    ...
    tps <- c(
      rep(1, length(ML2.key$unique.id[ML2.key$study.global.include[studies] == 1])),
    ...
    if (doAll) {
      studiess <- lapply(analysis[tp], function(c) ML2.key$unique.id[ML2.key[, c] >= 0])
    } else {
  ...
  if (any(is.na(studiess[!is.na(studiess) & studiess %in% studies]))) {
    stop("Analysis ID and analysis type do not agree [e.g. analysis type is 'primary', but analysis ID refers to 'secondary']")
  }
...
testScript <- function(studies,
  ...
  if (saveRDSfile) {
    fname <- c(
      file.path(normalizePath(dir.out), "RESULTS_RDS", subset, paste0("ML2_results_global_", subset, ".rds")),
      ...
      file.path(normalizePath(dir.out), "RESULTS_RDS", "by_order", paste0("Data_Figure_StudyOrder_", subset, ".rds"))
    )[tp]
  ...
  if (between(tp, 2, 3)) {
    ...
    l_ply(
      seq_along(dfout$aggregated),
      function(d) {
        if (!is.null(dfout$merged.results[[d]]) & NCOL(dfout$aggregated[[d]]) > 0) {
          rio::export(
            dfout$merged.results[[d]],
            paste0(
              dir.out, "/RESULTS_ANALYSIS/", subset, "/",
...
#' @examples
#' library(ggplot2)
#' g <- ggplot(data.frame(x = rnorm(n = 100), y = rnorm(n = 100)), aes(x = x, y = y)) +
#'   geom_point()
#' g + gg.theme()
#' g + gg.theme("noax")
#'
...
swarmPlot <- function(df, anonymous = FALSE, addSize = FALSE, addMedian = TRUE, addGlobalES = TRUE, addOriES = TRUE, addLabel = FALSE, oriES = NULL, fillvar = c("USA", "sigf")[1]) {
  ...
  btype <- "swarm"
  pdf(tempfile())
  bs <- beeswarm(ESCI.r ~ slabel,
    data = df,
    horizontal = FALSE, corral = "none",
    corralWidth = 5,
    pch = 21, spacing = 2, cex = .5,
    priority = "ascending", method = btype, do.plot = TRUE
  )[, c(1, 2, 4, 6)]
  dev.off()
  ...
  df$meanES <- plyr::ldply(
    unique(df$labels),
    function(r) {
      cbind(rep(
        mean(df$y[df$labels == r], na.rm = TRUE),
        sum(df$labels == r, na.rm = TRUE)
      ))
    }
  )[, 1]
  df$seES <- plyr::ldply(
  ...
  df$xx <- plyr::ldply(
    unique(df$labels),
    function(r) cbind(scale(df$x[df$labels == r], scale = F))
  )[, 1]
  df$xf <- plyr::ldply(
    seq_along(unique(df$labels)),
    function(r) {
      cbind(df$xx[df$labels == unique(df$labels)[r]] +
        seq(10, 10 * length(unique(df$labels)), by = 10)[r])
    }
  )[, 1]
  df$xn <- plyr::ldply(
    seq_along(unique(df$labels)),
    function(r) {
      cbind(rep(
        seq(10, 10 * length(unique(df$labels)), by = 10)[r],
        sum(df$labels == unique(df$labels)[r])
      ))
    }
  )[, 1]
...
get.plotly <- function(data, analysis_url) {
  p <- plot_ly(data,
    ...
    size = ESCI.N.total,
    text = paste(
      "<b>r</b> =", ESCI.r, "<b>p</b> <", signif(test.p.value, digits = 4), "<br>",
...
renderHTMLresults <- function(pageID) {
  rmarkdown::render(
    input = "ML2_interactive_results.Rmd",
    params = list(
...
any2any <- function(testInfo,
  ...
  if (is.null(st)) {
    stop("No test statistic to caclulate ES-CI.")
  }
  ...
  for (cnt in seq_along(sCI)) {
    ...
    switch(esType,
      ...
      t = esComp[[cnt]] <- compute.es::tes(
        t = x, level = CL * 100,
        n.1 = n1, n.2 = n2, verbose = FALSE, dig = 5
      ),
      lm.t = esComp[[cnt]] <- compute.es::a.tes(
        t = x, level = CL * 100,
        n.1 = n1, n.2 = n2, R = rID, q = q,
        verbose = FALSE, dig = 5
      ),
      t.r = esComp[[cnt]] <- compute.es::res(
        r = x, level = CL * 100, var.r = ((1 - x^2)^2) / (N - 1),
        n = N, verbose = FALSE, dig = 5
      ),
      r = esComp[[cnt]] <- compute.es::res(
        r = x, level = CL * 100, var.r = NULL,
        n = N, verbose = FALSE, dig = 5
      ),
      # compute.es::res(r=x, level=CL, n=N, verbose = FALSE, dig = 5),
      f = esComp[[cnt]] <- compute.es::fes(
        f = x, level = CL * 100,
        n.1 = n1, n.2 = n2, verbose = FALSE, dig = 5
      ),
      lm.f = esComp[[cnt]] <- compute.es::a.fes(
        f = x, level = CL * 100,
        n.1 = n1, n.2 = n2, R = rID, q = q,
        verbose = FALSE, dig = 5
      ),
      X2 = esComp[[cnt]] <- compute.es::chies(
        chi.sq = x, level = CL * 100,
        n = N, verbose = FALSE, dig = 5
      ),
...
get.ncpCI <- function(x, df1, df2, N, esType, CL = .95, keepSign = TRUE, keepDirection = TRUE, alternative = "two.sided") {
  ...
  switch(esType,
    t.p = ncCI <- list(
      Lower.Limit = MBESS::ci.sm(
        sm = x, alpha.lower = CLims[1], alpha.upper = CLims[2],
        conf.level = CL, N = N
      )$Lower.Conf.Limit.Standardized.Mean,
      Upper.Limit = MBESS::ci.sm(
        sm = x, alpha.lower = CLims[1], alpha.upper = CLims[2],
        conf.level = CL, N = N
      )$Upper.Conf.Limit.Standardized.Mean
    ),
    t = ncCI <- MBESS::conf.limits.nct(
      t.value = x, alpha.lower = CLims[1], alpha.upper = CLims[2], conf.level = CL,
      df = df1
    ),
    f = ncCI <- MBESS::conf.limits.ncf(
      F.value = x, alpha.lower = CLims[1], alpha.upper = CLims[2], conf.level = CL,
      df.1 = df1, df.2 = df2
    ),
    # t.r = ncCI <- MBESS::conf.limits.nct(t.value=x, conf.level=CL, df=df1),
    t.r = ncCI <- list(
      Lower.Limit = MBESS::ci.R(
        R = abs(x), alpha.lower = CLims[1], alpha.upper = CLims[2], conf.level = CL,
        N = N, K = 1
      )$Lower.Conf.Limit.R,
      Upper.Limit = MBESS::ci.R(
        R = abs(x), alpha.lower = CLims[1], alpha.upper = CLims[2], conf.level = CL,
        N = N, K = 1
      )$Upper.Conf.Limit.R
    ),
    X2 = ncCI <- MBESS::conf.limits.nc.chisq(
      Chi.Square = x, alpha.lower = CLims[1], alpha.upper = CLims[2], conf.level = CL,
      df = df1
    ),
    Z = ncCI <- list(
      Lower.Limit = (x + qnorm(CLimsZ[2], lower.tail = FALSE)),
  ...
  if (oneCor) {
    if ((dim(as.matrix(r1))[2] == 2)) {
      ...
      if (all((dim(as.matrix(r1)) != 1))) {
        ...
        disp(
          message = "- Either a single numerical value representing a correlation,",
          header = FALSE, footer = FALSE
        )
        disp(
          message = "- Or a 2 column matrix from which a correlation r1 can be calculated",
          header = FALSE
        )
        stop
      }
  ...
  if (length(vars) == 2) {
    if (any(plyr::laply(vars, is.factor))) {
      ...
      if (all(lengths(vars) > 1)) {
        ...
        longDat <- data.frame(xy = longDat$V1, group = factor(c(
          rep(1, length(vars[[1]])),
          rep(2, length(vars[[2]]))
        )))
      }
...
`%00%` <- function(x, y) {
  ...
  if (any(l0, isna, isnan, isinf, isnll)) {
    x <- y
  }
  return(x)
}
...
elascer <- function(x, mn = min(x, na.rm = T), mx = max(x, na.rm = T), lo = 0, hi = 1) {
  ...
  for (i in 1:NCOL(x)) {
    ...
    mx <- max(x[, i], na.rm = T)
    if (mn > mx) {
      warning("Minimum (mn) >= maximum (mx).")
    }
    if (lo > hi) {
      warning("Lowest scale value (lo) >= highest scale value (hi).")
    }
    ifelse(mn == mx,
      {
...
nlopt <- function(par, fn, lower, upper, control) {
  ...
  .nloptr <<- res <- nloptr::nloptr(par, fn,
    lb = lower, ub = upper,
    opts = list(
      algorithm = "NLOPT_LN_BOBYQA", print_level = 1,
...
varfun.Alter.1 <- function(vars) {
  ...
  cleanDataFilter <- data.frame(
    ...
    variable = c(
      rowSums(rbind(ok.Fluent[, id.Fluent.cols])),
      rowSums(rbind(ok.DisFluent[, id.DisFluent.cols]))
    ),
    factor = c(
      rep(names(vars[1]), NROW(vars[[1]])),
      rep(names(vars[2]), NROW(vars[[2]]))
    )
  )

  return(list(
    Fluent = rowSums(rbind(ok.Fluent[, id.Fluent.cols])),
...
varfun.Alter.2 <- function(vars) {
  ...
  return(list(
    Fluent = rowSums(ok.Fluent[, id.Fluent.cols]),
...
varfun.Miyamoto.1 <- function(vars) {
  ...
  return(list(
    Attitude = c(vars$CapitalCon[[1]], vars$CapitalPro[[1]]),
...
varfun.Inbar.1 <- function(vars) {
  ...
  return(list(
    r1 = cbind(vars$SameKiss["DSRs"], vars$SameKiss[outcome]),
...
varfun.Critcher.1 <- function(vars) {
  ...
  return(list(
    P97 = as.numeric(vars$P97$crit1.1),
...
varfun.vanLange.1 <- function(vars) {
  SVO <- rbind(
    cbind(c(85, 85), c(85, 76), c(85, 68), c(85, 59), c(85, 50), c(85, 41), c(85, 33), c(85, 24), c(85, 15)),
  ...
  SVO.index <- plyr::ldply(seq_along(id), function(s) {
    cbind(uID = vars$RawDataFilter[[1]]$uID[id[s]], SVO = atan(
...
varfun.Hauser.1 <- function(vars) {
  ...
  return(list(
    Response = factor(c(SE, GG), levels = c(1, 2), labels = vars$labels$Response),
...
varfun.Anderson.1 <- function(vars) {
  ...
  return(list(
    SWB = (MhighlowSW + MhighlowPA + MhighlowNAr) / 3,
...
varfun.Giessner.1 <- function(vars) {
  ...
  cleanDataFilter <- data.frame(
    uID = c(vars$Long$uID, vars$Short$uID),
    variable = c(rowMeans(Long[-1]), rowMeans(Short[-1])),
    factor = c(
      rep("Long", NROW(vars$Long)),
      rep("Short", NROW(vars$Short))
    )
  )

  return(list(
    Long = rowMeans(Long[-1]),
...
varfun.Hauser.2 <- function(vars) {
  ...
  return(list(
    Response = factor(c(SideEffect$haus3.1, GreaterGood$haus4.1), levels = c(1, 2), labels = vars$labels$Response),
    Condition = factor(c(rep(1, N[1]), rep(2, N[2])), levels = c(1, 2), labels = vars$labels$Condition),
    N = N,
    cleanDataFilter = cleanDataFilter
  ))
...
varfun.Risen.1 <- function(vars) {
  cleanDataFilter <- data.frame(
    uID = c(vars$Unprepared$uID, vars$Prepared$uID),
    variable = c(as.numeric(vars$Unprepared[[1]]), as.numeric(vars$Prepared[[1]])),
    factor = factor(c(
      rep("Unprepared", NROW(vars$Unprepared[[1]])),
      rep("Prepared", NROW(vars$Prepared[[1]]))
    ))
  )

  return(list(
    Unprepared = as.numeric(vars$Unprepared[[1]]),
...
varfun.Risen.2 <- function(vars) {
  ...
  return(list(
    Likelihood = c(vars$Unprepared$rise1.3, vars$Prepared$rise2.3),
...
varfun.Savani.1 <- function(vars) {
  ...
  Response <- rbind(
    dplyr::select(vars$Interpersonal, Choice = one_of(choice.Int)),
  ...
  Importance <- rbind(
    dplyr::select(vars$Interpersonal, Importance = one_of(importance.Int)),
    dplyr::select(vars$Personal, Importance = one_of(importance.Pers))
  )
  Importance$uID <- id
  Importance$Condition <- Condition
  Importance <- reshape2::melt(Importance,
    id = c("uID", "Condition"), variable.name = "VarLabel",
...
varfun.Norenzayan.1 <- function(vars) {
  cleanDataFilter <- data.frame(
    uID = c(vars$Belong$uID, vars$Similar$uID),
  ...
  return(list(
    Belong = rowMeans(vars$Belong == matrix(rep(1:2, 10), nrow = nrow(vars$Belong), ncol = 20, byrow = TRUE), na.rm = TRUE),
...
varfun.Norenzayan.2 <- function(vars) {
  ...
  return(list(
    Response = c(
      rowMeans(vars$Belong == matrix(rep(1:2, 10), nrow = nrow(vars$Belong), ncol = 20, byrow = TRUE), na.rm = TRUE),
...
varfun.Hsee.1 <- function(vars) {
  cleanDataFilter <- data.frame(
    uID = c(vars$Scarf$uID, vars$Coat$uID),
...
varfun.Gray.1 <- function(vars) {
  cleanDataFilter <- data.frame(
    uID = c(vars$adultHbaby$uID, vars$babyHadult$uID),
  ...
  return(list(
    adultHbaby = as.numeric(vars$adultHbaby[[1]]),
...
varfun.Zhong.1 <- function(vars) {
  ...
  cleanDataFilter <- data.frame(
    uID = c(vars$Ethical$uID, vars$Unethical$uID),
  ...
  return(list(
    Ethical = rowMeans(vars$Ethical[, idClean], na.rm = TRUE),
...
varfun.Zhong.3 <- function(vars) {
  ...
  return(list(
    Ethical = rowMeans(vars$Ethical[, idOther], na.rm = TRUE),
    Unethical = rowMeans(vars$Unethical[, idOther], na.rm = TRUE),
    N = c(
      length(rowMeans(vars$Ethical[, idOther], na.rm = TRUE)),
...
varfun.Schwarz.1 <- function(vars) {
  cleanDataFilter <- data.frame(
    ...
    factor = c(
      rep("SpecificFirst", nrow(vars$SpecificFirst)),
      rep("GlobalFirst", nrow(vars$GlobalFirst))
    )
  )

  return(list(
    r1 = cbind(vars$SpecificFirst[, 1], vars$SpecificFirst[, 2]),
...
varfun.Shafir.1 <- function(vars) {
  ...
  cleanDataFilter <- data.frame(
    ...
    parentB = rep(sum(
      c(
        sum(vars$Award$shaf1.1 == 2, na.rm = TRUE) / N[1],
...
varfun.Zaval.1 <- function(vars) {
  ...
  cleanDataFilter <- data.frame(
    uID = c(vars$Cold$uID[idC], vars$Heat$uID[idH]),
  ...
  return(list(
    Cold = as.numeric(unlist(vars$Cold[idC, 1])),
...
varfun.Knobe.1 <- function(vars) {
  cleanDataFilter <- data.frame(
    uID = c(vars$Help$uID, vars$Harm$uID),
...
varfun.Knobe.2 <- function(vars) {
  cleanDataFilter <- data.frame(
    uID = c(vars$Praise$uID, vars$Blame$uID),
...
varfun.Gati.1 <- function(vars) {
  ...
  CounterBalanceA <- list(
    P1st = c(2, 3, 5, 11, 12, 14, 16, 17, 18, 19, 22),
    P2nd = c(4, 6, 7, 8, 9, 10, 13, 15, 20, 21)
  )
  CounterBalanceB <- list(
    P1st = c(4, 6, 7, 8, 9, 10, 13, 15, 20, 21),
...
varfun.Gati.2 <- function(vars) {
  ...
  return(list(
    Asymmetry = df.subj.wide$Asymmetry,
    CompareTo = 0,
...
un.IT <- function(loose, unT = FALSE) {
  dp <- .packages()
  if (any(loose %in% dp)) {
    for (looseLib in loose[(loose %in% dp)]) {
      detach(paste0("package:", looseLib), unload = TRUE, character.only = TRUE)
    }
  }
  rm(dp)
  if (unT == TRUE) {
    ...
    if (any(loose %in% dp)) {
      remove.packages(loose[(loose %in% dp)])
    }
  }
}
